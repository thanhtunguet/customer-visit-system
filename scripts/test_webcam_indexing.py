...
async def test_webcam_indexing():
    ...
    working_devices = [
        idx for idx, info in available_devices.items() if info.get("is_working", False)
    ]
    ...
    success = streaming_service.start_stream(
        camera_id=test_camera_id, camera_type="webcam", device_index=test_device_index
    )
    ...
    if success:
        ...
        if (
            test_device_index in device_locks
            and device_locks[test_device_index] == test_camera_id
        ):
            print(
                f"✅ Device lock correctly set: device {test_device_index} → camera {test_camera_id}"
            )
        else:
            print(
                f"❌ Device lock not set correctly. Expected: {test_device_index} → {test_camera_id}, Got: {device_locks}"
            )
        ...
        if not conflict_success:
            print(
                f"✅ Conflict correctly detected when trying to use device {test_device_index}"
            )
        else:
            print(
                "❌ Conflict detection failed - should not allow multiple cameras on same device"
            )
    ...
    if len(working_devices) >= 2:
        ...
        success1 = streaming_service.start_stream(
            "camera1", "webcam", device_index=device1
        )
        success2 = streaming_service.start_stream(
            "camera2", "webcam", device_index=device2
        )
        ...
        if success1 and success2:
            ...
            print(
                f"❌ Failed to start simultaneous streams (device1: {success1}, device2: {success2})"
            )
    else:
        print(
            f"ℹ️ Only {len(working_devices)} working device(s) found - skipping multiple device test"
        )
