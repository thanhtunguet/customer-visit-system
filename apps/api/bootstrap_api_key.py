...
def main():
    ...
    try:
        ...
        print(f"\nVerification:")
        print(f"Tenant: {tenant_check.tenant_id} - {tenant_check.name}" if tenant_check else "Tenant not found")
        print(f"API Key: {api_key_check.name} - {api_key_check.role}" if api_key_check else "API key not found")
