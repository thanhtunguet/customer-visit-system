...
class User(Base):  # type: ignore[valid-type,misc]
    ...
    site_id = Column(
        BigInteger, ForeignKey("sites.site_id", ondelete="CASCADE"), nullable=True
    )  # For site_manager role
...
class Site(Base):  # type: ignore[valid-type,misc]
    ...
    cameras = relationship(
        "Camera", back_populates="site", cascade="all, delete-orphan"
    )
...
class Camera(Base):  # type: ignore[valid-type,misc]
    ...
    caps = Column(
        JSON, nullable=True
    )  # Camera capabilities: {"codec":"h264","res":"1920x1080","fps":25}
...
class CustomerFaceImage(Base):  # type: ignore[valid-type,misc]
    ...
    detection_metadata = Column(
        JSON, nullable=True
    )  # Additional metadata (detector, landmarks, etc.)
    ...
    __table_args__ = (
        ...
        Index(
            "idx_customer_face_images_confidence",
            "tenant_id",
            "customer_id",
            "confidence_score",
        ),
        ...
        Index(
            "idx_customer_face_images_created", "tenant_id", "customer_id", "created_at"
        ),
    )
