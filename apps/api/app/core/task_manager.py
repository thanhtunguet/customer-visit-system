...
class TaskManager:
    ...
    def __init__(self, max_concurrent_tasks: int = 50, db_task_pool_size: int = 5):
        ...
        self.thread_pool = ThreadPoolExecutor(max_workers=4, thread_name_prefix="TaskManager")
    ...
    def create_task(
        ...
        if len(self.active_tasks) >= self.max_concurrent_tasks:
            raise RuntimeError(f"Too many concurrent tasks ({self.max_concurrent_tasks})")
        
        if is_db_task and len(self.db_tasks) >= self.db_task_pool_size:
            raise RuntimeError(f"Too many concurrent DB tasks ({self.db_task_pool_size})")
        ...
        task.add_done_callback(lambda t: self._handle_task_completion(task_id, t, is_db_task))
    ...
    def _handle_task_completion(self, task_id: str, task: asyncio.Task, is_db_task: bool):
...
def create_task(coro, name: Optional[str] = None, timeout: Optional[float] = None) -> Optional[asyncio.Task]:
    """Create a managed task (convenience function)."""
    return task_manager.create_background_task(coro, name, is_db_task=False, timeout=timeout)


def create_db_task(coro, name: Optional[str] = None, timeout: Optional[float] = 30.0) -> Optional[asyncio.Task]:
    """Create a managed database task with default 30s timeout."""
    return task_manager.create_background_task(coro, name, is_db_task=True, timeout=timeout)
...
def create_broadcast_task(coro, name: Optional[str] = None) -> Optional[asyncio.Task]:
    ...
    return task_manager.create_background_task(coro, name, is_db_task=False, timeout=10.0)