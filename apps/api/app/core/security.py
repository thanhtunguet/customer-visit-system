...
def mint_jwt(
    sub: str, role: str, tenant_id: str, site_id: int = None, ttl_sec: int = 3600
) -> str:
...
def verify_jwt(token: str) -> Dict:
    ...
    try:
        key: Optional[str] = settings.jwt_public_key or (
            "dev-key" if not settings.jwt_private_key else None
        )
...
def require_system_admin(current_user: User = Depends(get_current_active_user)) -> User:
    ...
    if current_user.role != UserRole.SYSTEM_ADMIN:
        raise HTTPException(
            status_code=403, detail="System administrator access required"
        )
    return current_user
