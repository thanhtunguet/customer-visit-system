...
class EnhancedFaceProcessorService:
            ...
            async with self._initialization_lock:
                if self._face_processor is None:
                    try:
                        ...
                        logger.error(f"Failed to initialize enhanced face processor: {e}")
                        # Create mock processor for fallback
                        self._face_processor = MockFaceProcessor()
        
        return self._face_processor

    async def process_staff_image(self, image_bytes: bytes, staff_id: str) -> Dict[str, Any]:
        """
    ...
    async def assess_image_quality_only(self, image_bytes: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await face_processor.process_staff_image(
                image_bytes, 
                staff_id="quality_assessment_only"
            )
            ...
            if quality_score >= 0.8:
                ...
                assessment['upload_recommendation'] = 'Consider improving image before upload'
            else:
                assessment['quality_rating'] = 'Very Poor'
                assessment['upload_recommendation'] = 'Not recommended - please improve image quality'
    ...
    async def batch_process_staff_images(self, 
                                       images_data: List[tuple[bytes, str]], 
                                       quality_threshold: float = 0.5) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            return [{
                ...
                'quality_score': 0.0,
                'staff_id': images_data[i][1] if i < len(images_data) else 'unknown'
    ...
    async def get_processing_recommendations(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            if hasattr(face_processor, 'detector') and hasattr(face_processor.detector, 'detectors'):
                available_detectors = [name for name, _ in face_processor.detector.detectors]
...
class MockFaceProcessor:
    ...
    async def batch_process_staff_images(self, images_data: List[tuple]) -> List[Dict[str, Any]]:
