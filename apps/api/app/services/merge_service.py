...
class MergeService:
    ...
    async def execute_merge_visits_job(
        self, job: BackgroundJob, db_session: AsyncSession
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await db_session.execute(
                select(Visit).where(
                    and_(
                        Visit.tenant_id == job.tenant_id, Visit.visit_id.in_(visit_ids)
                    )
                )
            )
            ...
            if primary_visit_id:
                primary = next(
                    (v for v in visits if v.visit_id == primary_visit_id), None
                )
                if not primary:
                ...
                def _score(v: Visit):
                    return (
                        float(v.highest_confidence or v.confidence_score or 0.0),
            ...
            highest_confidence = max(
                float(v.highest_confidence or v.confidence_score or 0.0) for v in visits
            )
            ...
            for v in sorted(
                visits,
                key=lambda x: float(x.highest_confidence or x.confidence_score or 0.0),
                reverse=True,
            ):
                if v.image_path:
                    best_with_image = v
            ...
            self._update_job_progress(
                job.job_id, 60, "Reassigning customer face images"
            )
                ...
                await db_session.execute(
                    ...
                    .where(
                        and_(
                            ...
                            CustomerFaceImage.visit_id.in_(
                                [v.visit_id for v in non_primary_visits]
                            ),
                        )
                    )
            ...
            images_cleaned = await self._cleanup_minio_images_async(
                non_primary_with_images
            )
            ...
            if person_type == "customer":
                await self._recompute_customer_stats(
                    db_session, job.tenant_id, person_id
                )
                await db_session.commit()
    ...
    async def execute_cleanup_customer_faces_job(
        ...
        try:
            ...
            low_confidence_visits_result = await db_session.execute(
                select(Visit.visit_id, Visit.image_path)
                .where(
                    and_(
                        Visit.tenant_id == job.tenant_id,
                ...
                .order_by(Visit.confidence_score.asc())
                .limit(max_to_remove)
            )
            ...
            self._update_job_progress(
                job.job_id, 50, f"Removing {len(visit_ids_to_remove)} visits"
            )
    ...
    async def execute_bulk_delete_visits_job(
        ...
        try:
            ...
            self._update_job_progress(
                job.job_id, 20, f"Deleting {len(visit_ids)} visits"
            )

            result = await self._bulk_delete_visits(
                db_session, job.tenant_id, visit_ids
            )
    ...
    async def execute_bulk_merge_customers_job(
        ...
        try:
                ...
                try:
                    ...
                    for secondary_id in secondary_ids:
                        try:
                            ...
                            logger.error(
                                f"Failed to merge customer {secondary_id} into {primary_id}: {merge_error}"
                            )
                            merge_results.append(
                                {
                                    "primary_id": primary_id,
                    ...
                    completed_merges.append(
                        {
                            ...
                            "successful_merges": len(
                                [r for r in merge_results if r["status"] == "success"]
                            ),
                            "failed_merges": len(
                                [r for r in merge_results if r["status"] == "failed"]
                            ),
                        }
                    )
            ...
            total_successful_merges = sum(
                op.get("successful_merges", 0) for op in completed_merges
            )
            total_failed_merges = sum(
                op.get("failed_merges", 0) for op in completed_merges
            ) + len(failed_merges)
    ...
    async def _bulk_delete_visits(
        self, db_session: AsyncSession, tenant_id: str, visit_ids: List[str]
    ) -> Dict[str, Any]:
        ...
        visits_with_images_query = select(Visit.visit_id, Visit.image_path).where(
            and_(Visit.tenant_id == tenant_id, Visit.visit_id.in_(visit_ids))
        )
        ...
        visit_image_paths = [
            (vid, img_path) for vid, img_path in visits_with_images if img_path
        ]
        customer_image_tuples = [
            ("customer_face", path) for path in customer_face_image_paths if path
        ]
    ...
    async def _cleanup_minio_images_async(
        self, image_paths: List[Tuple[str, Optional[str]]]
    ) -> int:
    ...
    async def _recompute_customer_stats(
        self, db_session: AsyncSession, tenant_id: str, customer_id: int
    ):
    ...
    async def _execute_single_customer_merge(
        ...
        if (
            not primary_customer.estimated_age_range
            and secondary_customer.estimated_age_range
        ):
        ...
        try:
            ...
            await self._rebuild_customer_embeddings(
                db_session, tenant_id, primary_customer_id
            )
            ...
            logger.warning(
                f"Embedding maintenance failed during merge {primary_customer_id}<-{secondary_customer_id}: {e}"
            )
    ...
    async def _rebuild_customer_embeddings(
        ...
        for emb, timestamp in aggregated:
            try:
                ...
                logger.warning(
                    f"Failed to insert embedding for customer {customer_id}: {e}"
                )
