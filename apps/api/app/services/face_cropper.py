"""
Enhanced Face Cropping Utility for API
...
# For compatibility with API environment
...
class FaceCropper:
    """
    Enhanced face cropping utility with configurable parameters for API
    """
    ...
    def __init__(self,
        ...
        self.min_face_size = min_face_size or int(os.getenv('API_MIN_FACE_SIZE', '40'))
        self.crop_margin_pct = crop_margin_pct or float(os.getenv('API_CROP_MARGIN_PCT', '0.15'))
        self.target_size = target_size or int(os.getenv('API_TARGET_SIZE', '224'))
        self.max_face_ratio = max_face_ratio or float(os.getenv('API_MAX_FACE_RATIO', '0.6'))
        self.preserve_aspect = preserve_aspect if preserve_aspect is not None else os.getenv('API_PRESERVE_ASPECT', 'true').lower() == 'true'
        
        # Parse primary face strategy
        strategy_str = primary_face_strategy or os.getenv('API_PRIMARY_FACE_STRATEGY', 'best_quality')
        try:
            self.primary_face_strategy = PrimaryFaceStrategy(strategy_str)
        except ValueError:
            logger.warning(f"Invalid primary face strategy: {strategy_str}, using 'best_quality'")
            self.primary_face_strategy = PrimaryFaceStrategy.BEST_QUALITY
        
        logger.info(f"API FaceCropper initialized: min_face_size={self.min_face_size}, "
    ...
    def crop_face(self, image: ArrayType, face_info: Dict, debug: bool = False) -> Dict:
        """
    ...
    def crop_multiple_faces(self, image: ArrayType, faces: List[Dict], 
    ...
    def _crop_large_face(self, image: ArrayType, bbox: List[int], debug: bool) -> Dict:
    ...
    def _crop_small_face(self, image: ArrayType, bbox: List[int], debug: bool) -> Dict:
    ...
    def _crop_standard_face(self, image: ArrayType, bbox: List[int], debug: bool) -> Dict:
    ...
    def _resize_to_target(self, cropped_face: ArrayType, debug: bool) -> ArrayType:
        """
    ...
    def _select_primary_face(self, image: ArrayType, faces: List[Dict]) -> Dict:
        """
        Select primary face using configured strategy
        """
        if self.primary_face_strategy == PrimaryFaceStrategy.LARGEST:
            ...
            def distance_from_center(face):
                ...
                face_center_x = x + w / 2
                face_center_y = y + h / 2
                return ((face_center_x - img_center_x) ** 2 + (face_center_y - img_center_y) ** 2) ** 0.5
