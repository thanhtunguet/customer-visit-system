    ...
    async def request_worker_shutdown(
        ...
        try:
            async with get_db_session() as db:
                ...
                logger.info(
                    f"Shutdown requested for worker {worker_id} ({worker.worker_name}) by {requested_by}"
                )
        ...
        async with get_db_session() as db:
            ...
            if worker:
                ...
                try:
                    ...
                    logger.error(
                        f"Error broadcasting worker shutdown update: {broadcast_error}"
                    )
        ...
        async with get_db_session() as db:
            ...
            if worker:
                ...
                try:
                    ...
                    logger.error(
                        f"Error broadcasting worker timeout update: {broadcast_error}"
                    )

                logger.warning(
                    f"Forced worker {worker_id} offline due to shutdown timeout"
                )
