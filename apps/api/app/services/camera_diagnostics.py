...
class CameraDiagnostics:
                ...
                if cap.isOpened():
                    ...
                    if ret and frame is not None:
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Could not get additional properties for device {device_index}: {e}"
                            )

                        available_cameras[device_index] = device_info
                        logger.info(
                            f"Found working camera at index {device_index}: {width}x{height} @ {fps}fps"
                        )
                    else:
                        logger.warning(
                            f"Camera at index {device_index} opened but cannot capture frames"
                        )
    ...
    def generate_full_report() -> Dict:
        ...
        working_indices = [
            ...
            if info.get("is_working", False)
        ]

        if len(working_indices) == 0:
            report["recommendations"].append(
                "No working cameras found. Check camera connections."
            )
        elif len(working_indices) == 1:
            report["recommendations"].append(
                f"Only one working camera found at index {working_indices[0]}."
            )
        else:
            # Test simultaneous access
            simultaneous_test = CameraDiagnostics.test_simultaneous_access(
                working_indices
            )
            ...
            if not simultaneous_test.get("can_access_simultaneously", False):
                report["recommendations"].append(
                    "Cannot access multiple cameras simultaneously."
                )
            elif not simultaneous_test.get("feeds_are_different", False):
                report["recommendations"].append(
                    "Multiple camera indices point to the same physical camera."
                )
                report["recommendations"].append(
                    "This is a common issue on some systems where device enumeration is incorrect."
                )
            else:
                report["recommendations"].append(
                    "Multiple cameras are working correctly with different feeds."
                )
