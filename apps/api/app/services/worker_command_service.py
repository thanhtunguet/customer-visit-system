...
class WorkerCommandService:
    ...
    def send_command(
        ...
        if timeout_minutes:
            command_msg.expires_at = command_msg.created_at + timedelta(minutes=timeout_minutes)
        ...
        logger.info(f"Command {command.value} queued for worker {worker_id} with priority {priority.name}")
        return command_id
    
    def get_pending_commands(self, worker_id: str, limit: int = 10) -> List[WorkerCommandMessage]:
    ...
    def acknowledge_command(self, command_id: str, worker_id: str) -> bool:
        ...
        if worker_id in self.worker_queues:
            queue = self.worker_queues[worker_id]
            self.worker_queues[worker_id] = deque([cmd for cmd in queue if cmd.command_id != command_id])
    ...
    def get_worker_commands(self, worker_id: str, include_history: bool = False) -> List[WorkerCommandMessage]:
    ...
    def cancel_command(self, command_id: str, reason: str = "Cancelled by admin") -> bool:
