...
class AssignmentService:
    ...
    async def _find_available_cameras(
        self, db: AsyncSession, tenant_id: str, site_id: int
    ) -> List[Camera]:
        """
    ...
    async def _try_acquire_lease(
        self, db: AsyncSession, camera_id: int, worker_id: str, correlation_id: str
    ...
    async def _send_start_command(
        self, worker_id: str, camera: Camera, generation: int, correlation_id: str
    ):
    ...
    async def renew_lease(
        self, db: AsyncSession, worker_id: str, renewals: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        for renewal in renewals:
            ...
            try:
                ...
                result = await db.execute(
                    ...
                    .values(
                        lease_expires_at=new_expires_at, updated_at=datetime.utcnow()
                    )
                )
