...
class ImageProcessingService:
    ...
    async def generate_face_crop_from_bbox(
        ...
        try:
            ...
            return await loop.run_in_executor(
                ...
                bbox, frame_width, frame_height, padding_factor
            )
    ...
    def _generate_face_crop_sync(
        ...
        try:
            ...
            if crop_w < min_size or crop_h < min_size:
                logger.warning(f"Face crop too small: {crop_w}x{crop_h}, using default size")
            ...
            logger.debug(f"Generated face crop: {crop_w}x{crop_h}, {len(img_bytes)} bytes")
            return img_bytes
    ...
    def _create_face_placeholder(self, width: int, height: int, bbox: list[float]) -> Image.Image:
        ...
        try:
            ...
            draw.ellipse(
                ...
                fill='#f0f0f0', outline='#e8e8e8', width=1
            )
            ...
            draw.line(
                [(nose_x, nose_y - 3), (nose_x, nose_y + 3)], 
                fill='#e5e5e5', width=1
            )
    ...
    async def upload_generated_image(self, image_bytes: bytes, minio_client, bucket: str = "faces-derived") -> Optional[str]:
