...
class CameraStatusBroadcaster:
    ...
    def add_client(self, site_id: str, queue: asyncio.Queue):
        ...
        logger.info(f"Added SSE client for site {site_id}. Total clients: {len(self.connections[site_id])}")
    ...
    async def broadcast_camera_status_change(
        ...
        if (camera_key in self.last_status and 
