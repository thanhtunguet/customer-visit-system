...
class CustomerFaceService:
        ...
        try:
            ...
            result = await db.execute(
                select(Customer).where(
                    Customer.tenant_id == tenant_id, Customer.customer_id == customer_id
                )
            )
            ...
            if not customer:
                logger.warning(
                    f"Customer {customer_id} not found in tenant {tenant_id}, creating new customer"
                )
                ...
                logger.info(
                    f"Created missing customer {customer_id} for tenant {tenant_id}"
                )
            ...
            effective_threshold = (
                0.3 if is_manual_upload else self.min_confidence_to_save
            )

            if confidence_score < effective_threshold:
                logger.debug(
                    f"Face confidence {confidence_score:.3f} below threshold {effective_threshold:.3f} ({'manual' if is_manual_upload else 'auto'}), skipping"
                )
                return None
            ...
            if existing:
                logger.debug(
                    f"Duplicate face image detected for customer {customer_id}, skipping"
                )
                return existing

            # Calculate quality score
            quality_score = await self._calculate_quality_score(
                image_data, face_bbox, metadata
            )

            # Upload image to MinIO
            image_path = await self._upload_face_image(
                tenant_id, customer_id, image_data
            )
            if not image_path:
            ...
            logger.info(
                f"Added face image for customer {customer_id} with confidence {confidence_score:.3f}"
            )
            return face_image
            ...
            if 'relation "customer_face_images" does not exist' in str(e):
                logger.info(
                    f"customer_face_images table does not exist - cannot save face image for customer {customer_id}"
                )
                return None
            else:
    ...
    async def _upload_face_image(
        self, tenant_id: str, customer_id: int, image_data: bytes
    ) -> Optional[str]:
        ...
        try:
            ...
            filename = (
                f"customers/{tenant_id}/{customer_id}/face-{uuid.uuid4().hex[:8]}.jpg"
            )
    ...
    async def _manage_gallery_size(
        self, db: AsyncSession, tenant_id: str, customer_id: int
    ) -> None:
        ...
        try:
            ...
            if not customer:
                logger.warning(
                    f"Customer {customer_id} not found during gallery management, skipping"
                )
                return
            ...
            logger.info(
                f"Managing gallery size for customer {customer_id}: current={current_count}, max={self.max_images_per_customer}, removing={excess_count}"
            )
            ...
            result = await db.execute(
                ...
                .order_by(
                    ...
                    asc(
                        ...
                        + func.extract(
                            "epoch", func.now() - CustomerFaceImage.created_at
                        )
                        / 86400
                        * 0.01  # Small recency bonus
                    )
                )
                .limit(excess_count)
            )
            ...
            logger.info(
                f"Removed {len(worst_images)} excess face images for customer {customer_id}"
            )
    ...
    async def cleanup_excess_images_for_customer(
        self, db: AsyncSession, tenant_id: str, customer_id: int
    ) -> int:
        ...
        try:
            ...
            logger.info(
                f"Cleaned up {len(worst_images)} excess images for customer {customer_id} (had {current_count}, limit {self.max_images_per_customer})"
            )
            return len(worst_images)

        except Exception as e:
            logger.error(
                f"Error cleaning up excess images for customer {customer_id}: {e}"
            )
            await db.rollback()
            return 0

    async def cleanup_all_excess_images(
        ...
        try:
            ...
            result = await db.execute(
                ...
                .where(CustomerFaceImage.tenant_id == tenant_id)
                .group_by(CustomerFaceImage.customer_id)
                .having(
                    func.count(CustomerFaceImage.image_id)
                    > self.max_images_per_customer
                )
                .limit(limit)
            )
            ...
            for customer_row in customers_with_excess:
                ...
                cleaned = await self.cleanup_excess_images_for_customer(
                ...
                logger.info(
                    f"Customer {customer_id}: removed {cleaned} excess images (had {current_count})"
                )
    ...
    async def _delete_face_image_content_only(
        self, face_image: CustomerFaceImage
    ) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error deleting face image content {face_image.image_id}: {e}"
            )

    async def _delete_face_image(
        self, db: AsyncSession, face_image: CustomerFaceImage
    ) -> None:
    ...
    async def get_customer_face_images(
        ...
        try:
            ...
            if 'relation "customer_face_images" does not exist' in str(e):
                logger.info(
                    f"customer_face_images table does not exist - returning empty list for customer {customer_id}"
                )
                return []
            else:
                logger.error(
                    f"Error getting face images for customer {customer_id}: {e}"
                )
                return []
    ...
    async def get_best_customer_embeddings(
        ...
        try:
            images = await self.get_customer_face_images(
                db, tenant_id, customer_id, limit=max_embeddings
            )
            return [img.embedding for img in images if img.embedding]

        except Exception as e:
            logger.error(
                f"Error getting best embeddings for customer {customer_id}: {e}"
            )
            return []
