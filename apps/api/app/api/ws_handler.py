...
class IntentTracker:
    ...
    def handle_ack(
        self, intent_id: str, status: IntentStatus, details: Optional[str] = None
    ) -> bool:
...
class WorkerWebSocketHandler:
    ...
    async def _handle_messages(
        ...
        while True:
            try:
                ...
                correlation_id = message_dict.get(
                    "correlation_id", get_or_create_correlation_id()
                )
                ...
                logger.error("invalid_json", worker_id=worker_id, error=str(e))
                await self._send_error(websocket, "Invalid JSON format")

            except Exception as e:
                logger.error(
                    "message_handling_error", worker_id=worker_id, error=str(e)
                )
    ...
    async def _handle_register(
        self, websocket: WebSocket, worker_id: str, message: RegisterMessage
    ):
        ...
        try:
            ...
            worker_info = await worker_registry.register_worker(
                ...
                hostname=(
                    message.labels.get("hostname", "unknown")
                    if message.labels
                    else "unknown"
                ),
                ip_address=message.labels.get("ip_address") if message.labels else None,
                worker_name=(
                    message.labels.get("worker_name", worker_id)
                    if message.labels
                    else worker_id
                ),
    ...
    async def _handle_heartbeat(
        self, websocket: WebSocket, worker_id: str, message: HeartbeatMessage
    ):
            ...
            if message.renew:
                async with get_db() as db:
                    ...
                    result = await assignment_service.renew_lease(
                        db=db, worker_id=worker_id, renewals=renewal_requests
                    )
    ...
    async def _handle_ack(
        self, websocket: WebSocket, worker_id: str, message: AckMessage
    ):
        """Handle ACK message for intent tracking"""
        correlation_id = message.correlation_id or get_or_create_correlation_id()

        success = self.intent_tracker.handle_ack(
            intent_id=message.intent_id, status=message.status, details=message.details
        )
    ...
    async def _handle_event(
        self, websocket: WebSocket, worker_id: str, message: EventMessage
    ):
    ...
    async def send_start_command(
        ...
        if rtsp_url:
            ...
            source = CameraSource(
                type=SourceType.WEBCAM, device_index=device_index or 0
            )
