...
class WebRTCSignalingMessage(BaseModel):
    type: str = Field(..., description="Message type: offer, answer, ice-candidate, stream-request, stream-stop")
    ...
    ice_candidate: Optional[Dict[str, Any]] = Field(None, description="ICE candidate data")
...
class WebRTCSession:
    def __init__(self, session_id: str, client_id: str, worker_id: str, camera_id: int, site_id: int):
...
class WebRTCSignalingManager:
    ...
    async def connect_client(self, client_id: str, websocket: WebSocket, tenant_id: str):
    ...
    async def connect_worker(self, worker_id: str, websocket: WebSocket, tenant_id: str):
        """Register worker WebSocket connection"""
        await websocket.accept()
        self.worker_connections[worker_id] = websocket
        logger.info(f"Worker {worker_id} connected for WebRTC signaling in tenant {tenant_id}")
    ...
    async def create_streaming_session(self, client_id: str, camera_id: int, site_id: int, tenant_id: str) -> str:
        ...
        if not worker:
            raise HTTPException(status_code=404, detail="No worker available for camera")
        ...
        logger.info(f"Created WebRTC session {session_id}: client {client_id} -> worker {worker.worker_id} for camera {camera_id}")
        
        # Notify worker about new streaming session
        await self._send_to_worker(worker.worker_id, {
            "type": "stream-request",
            "session_id": session_id,
            "client_id": client_id,
            "camera_id": camera_id,
    ...
    async def handle_signaling_message(self, message: WebRTCSignalingMessage) -> bool:
        ...
        if message.from_id == session.client_id:
            ...
            logger.warning(f"Invalid sender {message.from_id} for session {message.session_id}")
            return False
        ...
        if token:
            try:
                ...
                if not worker or worker.tenant_id != tenant_id:
                    logger.warning(f"Worker {worker_id} not found or belongs to different tenant")
        ...
        while True:
            try:
                ...
                if message_data.get("type") == "signaling":
                    ...
                    logger.warning(f"Unknown message type from worker {worker_id}: {message_data.get('type')}")
