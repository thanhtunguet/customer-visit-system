...
async def renew_leases(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Lease renewal failed: {str(e)}"
        )
...
async def reclaim_expired_leases(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Lease reclaim failed: {str(e)}"
        )
...
async def get_lease_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get lease status: {str(e)}"
        )
...
async def cleanup_terminated_leases(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Lease cleanup failed: {str(e)}"
        )
...
async def manual_assign_camera(
    ...
    if worker.tenant_id != tenant_id:
        raise HTTPException(
            status_code=400,
            detail=f"Worker belongs to different tenant"
        )
    ...
    try:
        ...
        if camera:
            ...
            raise HTTPException(
                status_code=404,
                detail="No available cameras found for assignment"
            )
