...
async def get_upload_url(
    request: FileUploadRequest, user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        return FileUrlResponse(upload_url=upload_url, filename=request.filename)
...
async def get_download_url(
    request: FileDownloadRequest, user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        download_url = minio_client.get_presigned_url(
            bucket=bucket, object_name=object_name, expiry=timedelta(hours=1)
        )

        return FileDownloadResponse(
            download_url=download_url, filename=request.filename
        )
    except Exception as e:
        raise HTTPException(
