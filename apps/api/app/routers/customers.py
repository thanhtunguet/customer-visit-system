...
async def merge_customers(
    ...
    res = await db_session.execute(
        select(Customer.customer_id).where(
            and_(Customer.tenant_id == user["tenant_id"], Customer.customer_id.in_([primary_customer_id, secondary_customer_id]))
        )
    )
    ...
    async def _bg_merge():
        ...
        from ..models.database import Visit, CustomerFaceImage, Customer
        async with _db.get_session() as session:
            try:
                await _db.set_tenant_context(session, tenant_id)
                # Broadcast start
                await tenant_event_broadcaster.broadcast(tenant_id, {
                ...
                custs = await session.execute(
                    select(Customer).where(
                        and_(Customer.tenant_id == tenant_id, Customer.customer_id.in_([primary_customer_id, secondary_customer_id]))
                    )
                )
                ...
                await session.execute(
                    delete(Customer).where(and_(Customer.tenant_id == tenant_id, Customer.customer_id == secondary_customer_id))
                )
                ...
                try:
                    ...
                    for emb, created_at, ih in gres.all():
                        if not emb or not isinstance(emb, list) or len(emb) != 512:
                            continue
                        key = f"img:{ih}" if ih else f"vec:{hashlib.sha256(str(emb).encode()).hexdigest()}"
