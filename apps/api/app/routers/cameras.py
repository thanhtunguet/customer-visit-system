...
async def create_camera(
    ...
    if camera.camera_type.value == "webcam":
        if camera.device_index is None:
            raise HTTPException(
                status_code=400, detail="device_index is required for webcam cameras"
            )
        if camera.device_index < 0:
            raise HTTPException(
                status_code=400, detail="device_index must be non-negative"
            )
        ...
        if not camera.rtsp_url:
            raise HTTPException(
                status_code=400, detail="rtsp_url is required for RTSP cameras"
            )
        if camera.device_index is not None:
            raise HTTPException(
                status_code=400,
                detail="device_index should not be set for RTSP cameras",
            )
...
@router.get(
    "/sites/{site_id:int}/cameras/{camera_id:int}", response_model=CameraResponse
)
...
async def delete_camera(
    ...
    try:
        if camera_proxy_service.is_camera_streaming(camera_id):
            ...
            if stop_result.get("success"):
                ...
                logger.warning(
                    f"Failed to stop stream for camera {camera_id}: {stop_result.get('error')}"
                )
    ...
    response = {"message": "Camera deleted successfully", "camera_id": camera_id}
    if stopped_services:
        response["stopped_services"] = stopped_services
        logger.info(
            f"Camera {camera_id} deleted after stopping services: {stopped_services}"
        )
...
async def start_camera_stream(
    ...
    # Debug logging
    logger.info(
        f"Starting camera stream - tenant_id: {tenant_id} (type: {type(tenant_id)}), site_id: {site_id} (type: {type(site_id)}), camera_id: {camera_id} (type: {type(camera_id)})"
    )
...
async def stop_camera_processing(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to stop processing: {str(e)}"
        )
...
async def get_site_streaming_status(
    ...
    result = await db_session.execute(
        select(Camera).where(
            and_(Camera.tenant_id == tenant_id, Camera.site_id == site_id)
        )
    )
    cameras = result.scalars().all()
    ...
    for camera in cameras:
        ...
        camera_status = await camera_proxy_service.get_camera_stream_status(
            camera.camera_id
        )
        ...
        if worker_info:
            if worker_id not in streaming_status["workers"]:
                ...
                if (
                    ...
                    and "active_camera_streams" in worker_info.capabilities
                ):
                    worker_streaming_info["active_camera_streams"] = (
                        worker_info.capabilities["active_camera_streams"]
                    )
                    worker_streaming_info["total_active_streams"] = (
                        worker_info.capabilities.get("total_active_streams", 0)
                    )

                streaming_status["workers"][worker_id] = worker_streaming_info

            # Add this camera to worker's assigned cameras list
            streaming_status["workers"][worker_id]["assigned_cameras"].append(
                camera.camera_id
            )

            # Check for worker issues
            if not worker_info.is_healthy or worker_info.status not in [
                "idle",
                "processing",
                "online",
            ]:
...
async def get_streaming_overview(user: Dict = Depends(get_current_user)):
    ...
    overview = {
        ...
        "workers": [],
        "camera_assignments": camera_delegation_service.list_assignments(
            tenant_id=tenant_id
        ),
        "summary": {
            "healthy_workers": 0,
    ...
    for worker in tenant_workers:
        ...
        # Get streaming info from worker capabilities
        if worker.capabilities and "active_camera_streams" in worker.capabilities:
            worker_info["active_camera_streams"] = worker.capabilities[
                "active_camera_streams"
            ]
            worker_info["total_active_streams"] = worker.capabilities.get(
                "total_active_streams", 0
            )

            if worker_info["total_active_streams"] > 0:
                overview["summary"]["workers_with_active_streams"] += 1
                overview["summary"]["total_active_streams"] += worker_info[
...
async def camera_status_stream(
    site_id: int, request: Request, user: Dict = Depends(get_current_user_for_stream)
):
...
async def cleanup_all_streams(user: Dict = Depends(get_current_user)):
...
async def stop_all_camera_jobs(camera_id: int, user: Dict = Depends(get_current_user)):
...
async def validate_device_consistency(user: Dict = Depends(get_current_user)):
    ...
    result = await db_session.execute(
        select(Camera).where(
            and_(Camera.tenant_id == user["tenant_id"], Camera.camera_type == "webcam")
        )
    )
    ...
    for camera in webcam_cameras:
        mapping = {
            ...
            "device_working": (
                available_devices.get(camera.device_index, {}).get("is_working", False)
                if camera.device_index in available_devices
                else False
            ),
        }
        ...
        if camera.device_index not in available_devices:
            validation_report["issues"].append(
                f"Camera {camera.camera_id} ({camera.name}) uses device_index {camera.device_index} which is not available in OpenCV enumeration"
            )
            ...
            validation_report["issues"].append(
                f"Camera {camera.camera_id} ({camera.name}) uses device_index {camera.device_index} which exists but is not working"
            )
            validation_report["cameras_using_invalid_devices"].append(
                {
                    "camera_id": camera.camera_id,
    ...
    for camera in webcam_cameras:
        if camera.device_index in device_usage:
            validation_report["issues"].append(
                f"Device index {camera.device_index} is used by multiple cameras: {device_usage[camera.device_index]} and {camera.camera_id}"
            )
        else:
    ...
    validation_report["summary"] = {
        ...
        "working_opencv_devices": len(
            [d for d in available_devices.values() if d.get("is_working", False)]
        ),
...
async def run_camera_diagnostics(user: Dict = Depends(get_current_user)):
...
async def list_available_webcams(
    site_id: Optional[int] = None, user: Dict = Depends(get_current_user)
):
    """
    ...
    if site_id:
        site_workers = worker_registry.list_workers(
