...
class WorkerRegistrationRequest(BaseModel):
    worker_id: Optional[str] = Field(
        None, description="Preferred worker ID (from env or .env)"
    )
    ...
    site_id: Optional[int] = Field(
        None, description="Site assignment for camera allocation"
    )
    is_reconnection: Optional[bool] = Field(
        False, description="Flag indicating reconnection"
    )


class WorkerHeartbeatRequest(BaseModel):
    status: str = Field(
        ..., description="Worker status: idle, processing, offline, error"
    )
    faces_processed_count: Optional[int] = Field(
        0, description="Faces processed since last heartbeat"
    )
    ...
    current_camera_id: Optional[int] = Field(
        None, description="Currently processing camera"
    )
...
class CameraAssignmentRequest(BaseModel):
    site_id: Optional[int] = Field(
        None, description="Preferred site for camera assignment"
    )
...
class ShutdownRequest(BaseModel):
    signal: str = Field(
        default="graceful", description="Shutdown type: graceful, immediate, restart"
    )
...
async def register_worker(
    ...
    if current_user.role not in ["worker", "system_admin", "tenant_admin"]:
        raise HTTPException(
            status_code=403, detail="Only workers or admins can register"
        )
...
async def request_camera_assignment(
    ...
    return CameraAssignmentResponse(
        ...
        site_id=worker.site_id,
        message=(
            "Camera assignment handled during registration"
            if worker.camera_id
...
async def receive_stop_signal(
    ...
    if current_user.role not in ["worker", "system_admin"]:
        raise HTTPException(
            status_code=403, detail="Only workers can send stop signals"
        )
...
async def deregister_worker(
    ...
    if current_user.role not in ["system_admin", "tenant_admin", "worker"]:
        raise HTTPException(
            status_code=403, detail="Only admins or workers can deregister"
        )
