...
class WorkerCommandRequest(BaseModel):
    ...
    priority: str = Field("normal", description="Command priority: low, normal, high, urgent")
    timeout_minutes: Optional[int] = Field(None, description="Command timeout in minutes")
...
async def assign_camera_to_worker(
    ...
    if current_user.role not in ["system_admin", "tenant_admin", "site_manager"]:
        raise HTTPException(
            status_code=403,
            detail="Only admins can assign cameras to workers"
        )
...
async def release_camera_from_worker(
    ...
    if current_user.role not in ["system_admin", "tenant_admin", "site_manager"]:
        raise HTTPException(
            status_code=403,
            detail="Only admins can release cameras from workers"
        )
...
async def send_command_to_worker(
    ...
    if current_user.role not in ["system_admin", "tenant_admin", "site_manager"]:
        raise HTTPException(
            status_code=403,
            detail="Only admins can send commands to workers"
        )
...
async def list_camera_assignments(
    ...
    assignments = camera_delegation_service.list_assignments(tenant_id=current_user.tenant_id)
...
async def cleanup_stale_assignments(
    ...
    if current_user.role not in ["system_admin", "tenant_admin"]:
        raise HTTPException(
            status_code=403,
            detail="Only admins can cleanup stale assignments"
        )
...
async def auto_assign_cameras(
    ...
    if current_user.role not in ["system_admin", "tenant_admin"]:
        raise HTTPException(
            status_code=403,
            detail="Only admins can trigger auto-assignment"
        )
...
async def get_pending_commands_for_worker(
    ...
    if current_user.role not in ["worker", "system_admin"]:
        raise HTTPException(
            status_code=403,
            detail="Only workers or admins can get pending commands"
        )
...
async def acknowledge_command(
    ...
    if current_user.role not in ["worker", "system_admin"]:
        raise HTTPException(
            status_code=403,
            detail="Only workers can acknowledge commands"
        )
...
async def complete_command(
    ...
    if current_user.role not in ["worker", "system_admin"]:
        raise HTTPException(
            status_code=403,
            detail="Only workers can complete commands"
        )
...
async def cancel_command(
    ...
    if current_user.role not in ["system_admin", "tenant_admin"]:
        raise HTTPException(
            status_code=403,
            detail="Only admins can cancel commands"
        )
    ...
    try:
        ...
        if success:
            return {"message": "Command cancelled", "command_id": command_id, "reason": reason}
        else:
            raise HTTPException(status_code=404, detail="Command not found")
