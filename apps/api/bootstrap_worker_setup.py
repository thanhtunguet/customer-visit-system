...
def setup_worker_data():
    ...
    try:
        ...
        print(
            f"Connecting to PostgreSQL: host={host}, port={port}, user={user}, database={database}"
        )
        ...
        """,
            (
                tenant_id,
                tenant_name,
                "Development tenant for workers",
                True,
                datetime.utcnow(),
                datetime.utcnow(),
            ),
        ...
        # Verify the setup
        cur.execute(
            "SELECT tenant_id, name FROM tenants WHERE tenant_id = %s", (tenant_id,)
        )
        tenant_row = cur.fetchone()
        print(f"✅ Tenant verified: {tenant_row}")

        cur.execute(
            "SELECT name, role FROM api_keys WHERE tenant_id = %s AND hashed_key = %s",
            (tenant_id, key_hash),
        )
