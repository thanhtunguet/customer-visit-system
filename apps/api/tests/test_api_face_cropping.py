...
from apps.api.app.services.face_cropper import (FaceCropper,
                                                PrimaryFaceStrategy,
                                                api_face_cropper)
...
class TestFaceProcessingServiceIntegration:
    ...
    def test_enhanced_detect_faces_and_landmarks(
        self, face_service, sample_base64_image
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_staff_face_image_enhanced(
        self, face_service, sample_base64_image
    ):
        """Test enhanced staff face processing"""
        with (
            patch.object(
                face_service, "_enhanced_detect_faces_and_landmarks"
            ) as mock_detect,
    ...
    @pytest.mark.asyncio
    async def test_process_staff_face_image_enhanced_multi_face(
        self, face_service, sample_base64_image
    ):
        ...
        with (
            ...
            assert result["crop_metadata"]["selection_strategy"] in [
                "best_quality",
                "largest",
                "most_centered",
                "highest_confidence",
    ...
    @pytest.mark.asyncio
    async def test_process_customer_faces_from_image_enhanced(
        self, face_service, sample_base64_image
    ):
    ...
    @pytest.mark.asyncio
    async def test_enhanced_processing_fallback(
        self, face_service, sample_base64_image
    ):
        ...
        with (
            patch.object(
                face_service,
                "_enhanced_detect_faces_and_landmarks",
    ...
    def test_extract_face_region_fallback(self, face_service, sample_base64_image):
        ...
        with patch(
            "app.services.face_processing_service.api_face_cropper"
        ) as mock_cropper:
...
class TestApiFaceCroppingIntegration:
    ...
    def create_test_image_with_faces(self, face_positions: List[dict]) -> str:
        ...
        for i, face_spec in enumerate(face_positions):
            ...
            # Draw face
            cv2.rectangle(
                image, (x, y), (x + face_size, y + face_size), (150, 150, 150), -1
            )
            cv2.circle(
                image,
            ...
            cv2.ellipse(
                image,
                (x + face_size // 2, y + 2 * face_size // 3),
                (face_size // 6, face_size // 8),
                0,
                0,
                180,
                (100, 100, 100),
    ...
    async def test_multi_face_api_scenarios(self):
        ...
        with (
            ...
            for face in result["faces"]:
                assert "crop_metadata" in face
                assert face["crop_metadata"]["crop_strategy"] in [
                    "standard",
                    "small_face",
                    "large_face",
                ]
