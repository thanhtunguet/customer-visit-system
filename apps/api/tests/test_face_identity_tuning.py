...
async def test_low_quality_rejected(monkeypatch):
    ...
    with patch("apps.api.app.services.face_service.milvus_client.search_similar_faces", new=AsyncMock(return_value=[])):
...
async def test_prefers_known_with_soft_threshold_and_margin(monkeypatch):
    ...
    with patch("apps.api.app.services.face_service.milvus_client.search_similar_faces", new=AsyncMock(return_value=matches)):
        with patch("apps.api.app.services.face_service.milvus_client.insert_embedding", new=AsyncMock(return_value="ok")):
            db_session = AsyncMock()
            res = await face_service.process_face_event(evt, db_session, tenant_id="t1")
