...
def test_detect_faces_and_landmarks_success(
    mock_detector_class, face_processing_service
):
...
async def test_upload_image_to_minio_success(face_processing_service):
    ...
    with patch(
        "app.services.face_processing_service.minio_client.upload_file"
    ) as mock_upload:
...
@pytest.mark.asyncio
async def test_process_staff_face_image_success(
    face_processing_service, sample_base64_image
):
    ...
    with patch.object(
        face_processing_service, "detect_faces_and_landmarks", return_value=mock_faces
    ):
        with patch.object(
            face_processing_service, "extract_face_embedding", return_value=[0.1] * 512
        ):
            with patch.object(
                face_processing_service,
                "upload_image_to_minio",
                return_value="test-path.jpg",
            ):
...
@pytest.mark.asyncio
async def test_process_staff_face_image_no_faces(
    face_processing_service, sample_base64_image
):
    """Test face processing when no faces are detected."""
    with patch.object(
        face_processing_service, "detect_faces_and_landmarks", return_value=[]
    ):
        result = await face_processing_service.process_staff_face_image(
            sample_base64_image, "tenant-123", 456
        )
...
@pytest.mark.asyncio
async def test_test_face_recognition_success(
    face_processing_service, sample_base64_image
):
    ...
    assert (
        result["matches"][0]["staff_id"] == 123
    )  # John Doe should be first (better match)
...
@pytest.mark.asyncio
async def test_test_face_recognition_no_face_detected(
    face_processing_service, sample_base64_image
):
