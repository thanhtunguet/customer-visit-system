...
class WorkerClient:
    ...
    def _parse_site_id(self) -> Optional[int]:
        ...
        logger.warning(
            f"Invalid site_id format: {site_id_str}, expected integer or 's-N' format"
        )
        return None
    ...
    async def initialize(self):
        ...
        if env_worker_id:
            ...
            logger.info(
                "No WORKER_ID environment variable set, will use .env file or auto-generate"
            )
    ...
    async def shutdown(self):
        ...
        if self.worker_id:
            ...
            for attempt in range(max_attempts):
                try:
                    logger.info(
                        f"Sending offline status (attempt {attempt + 1}/{max_attempts})"
                    )
                    ...
                    logger.warning(
                        f"Timeout sending offline status (attempt {attempt + 1})"
                    )
                    if attempt == max_attempts - 1:
                        logger.warning("Max attempts reached, proceeding with shutdown")
                except Exception as e:
                    logger.warning(
                        f"Failed to send offline status (attempt {attempt + 1}): {e}"
                    )
                    if attempt == max_attempts - 1:
                        logger.warning("Max attempts reached, proceeding with shutdown")
    ...
    async def _register_worker(self):
        ...
        for attempt in range(self.max_registration_retries):
            try:
                ...
                registration_data = {
                    ...
                    "is_reconnection": self.worker_id
                    is not None,  # Flag for API to know this is a reconnection
                }
                ...
                if backend_worker_id and backend_worker_id != intended_worker_id:
                    logger.warning(
                        f"Backend returned different worker_id ({backend_worker_id}), keeping intended ID ({intended_worker_id})"
                    )
                ...
                if "assigned_camera_id" in result and result["assigned_camera_id"]:
                    ...
                    logger.info(
                        f"Worker assigned camera {self.assigned_camera_id} during registration"
                    )
                else:
                ...
                status = (
                    "reconnected"
                    if registration_data["is_reconnection"]
                    else "registered"
                )
                logger.info(
                    f"Worker {status} successfully with consolidated API - ID {self.worker_id}: {result.get('message', 'Success')}"
                )
                return

            except Exception as e:
                logger.error(
                    f"Worker registration failed (attempt {attempt + 1}/{self.max_registration_retries}): {e}"
                )

                # If we're using a persistent worker ID and getting 404/403 errors,
                # it might be stale - try clearing it on the last attempt
                if (
                    attempt == self.max_registration_retries - 1
                    and intended_worker_id
                    and hasattr(e, "response")
                    and e.response
                    and e.response.status_code in [404, 403]
                ):
                    logger.warning(
                        "Clearing potentially stale worker ID for fresh registration"
                    )
                    worker_id_manager.clear_worker_id()
                    ...
                    intended_worker_id = worker_id_manager.get_or_create_worker_id(
                        tenant_id=self.config.tenant_id,
                        site_id=(
                            str(self.config.site_id) if self.config.site_id else None
                        ),
                    )
        ...
        logger.error(
            f"Failed to register worker after {self.max_registration_retries} attempts"
        )
        ...
        try:
            ...
            if backend_camera_id != self.assigned_camera_id:
                if backend_camera_id:
                    logger.info(
                        f"Backend updated camera assignment: {self.assigned_camera_id} → {backend_camera_id}"
                    )
            ...
            logger.debug(
                f"Heartbeat sent successfully to consolidated API - status: {status.value}"
            )
    ...
    async def report_error(self, error_message: str):
        ...
        await self._send_heartbeat(
            status=WorkerStatus.ERROR, error_message=error_message
        )
    ...
    async def request_camera_assignment(self):
        ...
        try:
            ...
            if result.get("assigned") and result.get("camera_id"):
                ...
                logger.info(
                    f"Camera assignment requested successfully: {self.assigned_camera_id} ({self.assigned_camera_name})"
                )
                return self.assigned_camera_id
            else:
                logger.info(
                    f"Camera assignment requested but not available: {result.get('message')}"
                )
                return None

        except Exception as e:
            logger.error(
                f"Failed to request camera assignment from consolidated API: {e}"
            )
            return None
        ...
        try:
            ...
            if result.get("has_shutdown_signal"):
                ...
                if shutdown_signal:
                    ...
                    logger.warning(
                        f"Shutdown signal received: {shutdown_signal['signal']}"
                    )
    ...
    async def _handle_assign_camera_command(self, parameters: dict) -> Dict[str, Any]:
        ...
        if (
            hasattr(self, "_camera_assignment_callback")
            and self._camera_assignment_callback
        ):
            try:
                await self._camera_assignment_callback(
                    camera_id, camera_type, rtsp_url, device_index
                )
    ...
    async def _handle_start_processing_command(
    ...
    async def _handle_stop_processing_command(
    ...
    async def _handle_start_streaming_command(
        self, _parameters: dict
    ) -> Dict[str, Any]:
