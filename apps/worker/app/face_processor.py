...
class FaceProcessor:
    ...
    async def process_staff_image(
        self, image_data: bytes, staff_id: str
    ) -> Dict[str, Any]:
        """
    ...
    async def _assess_image_quality(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        def assess_quality():
            ...
            if min(height, width) < min_resolution:
                issues.append(
                    f"Low resolution ({width}x{height}, recommended: ≥{min_resolution}x{min_resolution})"
                )
            ...
            )  # Weight less than other factors
    ...
    async def _select_best_face(self, faces: List[Dict], image: np.ndarray) -> Dict:
        ...
        logger.debug(
            f"Selected best face with score {scored_faces[0][1]:.3f} using {best_face.get('detector', 'unknown')} detector"
        )
    ...
    async def _calculate_final_quality(
        self, face_info: Dict, image_quality: Dict, embedding: List[float]
    ) -> float:
    ...
    def _get_quality_suggestions(self, quality_assessment: Dict) -> List[str]:
        ...
        if min(quality_assessment["dimensions"]) < 200:
            suggestions.append(
                "Use higher resolution image (recommended: 400x400 or larger)"
            )
