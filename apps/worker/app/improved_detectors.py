...
class ImprovedFaceDetector:
        ...
        try:
            ...
            for key, result in results.items():
                ...
                if confidence >= self.confidence_threshold:
                    ...
                    # Extract landmarks
                    landmarks = []
                    for landmark_key in ['left_eye', 'right_eye', 'nose', 'mouth_left', 'mouth_right']:
    ...
    def _filter_overlapping_faces(self, faces: List[Dict], overlap_threshold: float = 0.3) -> List[Dict]:
        ...
        for face in faces:
            ...
            for existing_face in filtered_faces:
                if self._calculate_overlap(face['bbox'], existing_face['bbox']) > overlap_threshold:
                    is_duplicate = True
                    break
    ...
    def detect(self, image: np.ndarray) -> List[Dict]:
        ...
        for preprocess_name, processed_image in preprocessed_images:
            ...
            for detector_name, detector in self.detectors:
                try:
                    ...
                    if faces:
                        logger.debug(f"{detector_name} on {preprocess_name}: found {len(faces)} faces")
                        ...
                        if any(f.get('landmarks') for f in faces):
                            logger.debug(f"Found faces with landmarks using {detector_name}")
            ...
            if any(f.get('landmarks') and f.get('confidence', 0) > 0.7 for f in all_faces):
                break
        
        # Filter overlapping detections and return the best ones
        filtered_faces = self._filter_overlapping_faces(all_faces, overlap_threshold=0.5)
        ...
        logger.info(f"Enhanced detection found {len(filtered_faces)} high-quality faces")
        return filtered_faces
...
class ImprovedFaceEmbedder:
    ...
    def _initialize(self):
        ...
        logger.warning("No face embedding models available, using enhanced mock embedder")
        self.model = None
    ...
    def enhanced_face_alignment(self, image: np.ndarray, landmarks: np.ndarray) -> np.ndarray:
        """
