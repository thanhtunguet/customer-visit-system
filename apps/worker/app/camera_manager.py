...
class CameraManager:
    ...
    async def start_camera(self, camera_id: int, camera_config: Dict[str, Any]) -> bool:
        ...
        if (
            camera_id in self.frame_buffers
            and self.frame_buffers[camera_id] is not None
        ):
    ...
    def _camera_capture_thread(self, camera_id: int, camera_config: Dict[str, Any]):
        ...
        try:
            ...
            if camera_type == "rtsp":
                ...
                logger.info(
                    f"Camera {camera_id}: Opening webcam device: {device_index}"
                )
            ...
            logger.info(
                f"Camera {camera_id}: Configured for {width}x{height} at {fps} FPS"
            )
            ...
            while not self.stop_flags[camera_id].is_set():
                ...
                if current_time - last_stats_update >= 1.0:
                    ...
                    logger.debug(
                        f"Camera {camera_id}: capture FPS ~{stats['fps']:.1f}, last_frame_age={(time.time()-stats['last_frame_time']):.2f}s"
                    )
