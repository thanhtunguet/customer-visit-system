...
class FaceCropper:
    ...
    def __init__(self,
        ...
        self.crop_margin_pct = crop_margin_pct or float(os.getenv('CROP_MARGIN_PCT', '0.15'))
        self.target_size = target_size or int(os.getenv('TARGET_SIZE', '224'))
        self.max_face_ratio = max_face_ratio or float(os.getenv('MAX_FACE_RATIO', '0.6'))
        self.preserve_aspect = preserve_aspect if preserve_aspect is not None else os.getenv('PRESERVE_ASPECT', 'true').lower() == 'true'
        
        # Parse primary face strategy
        strategy_str = primary_face_strategy or os.getenv('PRIMARY_FACE_STRATEGY', 'best_quality')
        try:
        ...
        logger.info(f"FaceCropper initialized: min_face_size={self.min_face_size}, "
    ...
    def crop_face(self, image: np.ndarray, face_info: Dict, debug: bool = False) -> Dict:
        """
    ...
    def crop_multiple_faces(self, image: np.ndarray, faces: List[Dict], 
    ...
    def _crop_standard_face(self, image: np.ndarray, bbox: List[int], debug: bool) -> Dict:
