...
class WorkerFSM:
    ...
    def __init__(self, worker_id: str):
        ...
        logger.info(
            f"WorkerFSM initialized for {worker_id} in state {self.state.value}"
        )

    def transition_to(
        self, new_state: WorkerState, reason: Optional[str] = None
    ) -> bool:
        """
    ...
    def on_start_camera(self, camera_id: str):
        ...
        if not self.can_start_camera():
            logger.warning(
                f"FSM {self.worker_id}: Cannot start camera in state {self.state.value}"
            )
            return False
    ...
    def on_pipeline_ready(self):
        ...
        if self.state == WorkerState.RUNNING:
            logger.info(
                f"FSM {self.worker_id}: Pipeline ready for camera {self.camera_id}"
            )
...
class PipelineSupervisor:
    ...
    def start_pipeline(self, camera_id: str):
        ...
        logger.info(
            f"Pipeline {self.worker_id}: Starting pipeline for camera {camera_id}"
        )
    ...
    def stop_pipeline(self):
        ...
        if self.active_camera:
            logger.info(
                f"Pipeline {self.worker_id}: Stopping pipeline for camera {self.active_camera}"
            )
