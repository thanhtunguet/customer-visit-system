...
async def simulate_event_with_image(token: str, client: httpx.AsyncClient) -> None:
    ...
    # Face outline
    draw.ellipse(
        [face_margin, face_margin, face_margin + face_size, face_margin + face_size],
        fill="peachpuff",
    ...
    draw.polygon(
        [(nose_x, nose_y - 8), (nose_x - 5, nose_y + 5), (nose_x + 5, nose_y + 5)],
        fill="rosybrown",
        outline="sienna",
    )
    ...
    draw.arc(
        [mouth_left, mouth_y, mouth_right, mouth_y + mouth_h],
        start=0,
        end=180,
        fill="darkred",
        width=3,
    ...
    face_bbox = [
        ...
        face_size,
        face_size,
    ]  # x, y, w, h matching the drawn face
    ...
    evt = FaceDetectedEvent(
        ...
        embedding=[
            np.random.random() for _ in range(512)
        ],  # Random but realistic embedding
    ...
    for attempt in range(max_retries):
        try:
            ...
            data = {"event_data": json.dumps(event_dict)}
            ...
            if r.status_code == 200:
                ...
                print(
                    f"✅ Event processed: person_id={response_data.get('person_id')}, match={response_data.get('match')}"
                )
...
class WorkerConfig:
    ...
    def __init__(self):
        ...
        self.site_id = (
            int(site_id_str.split("-")[1])
            if site_id_str.startswith("s-")
            else int(site_id_str)
        )
        ...
        self.embedder_type = os.getenv(
            "EMBEDDER_TYPE", "insightface"
        )  # insightface, mock
        self.worker_fps = int(os.getenv("WORKER_FPS", "5"))
        self.confidence_threshold = float(
            os.getenv("CONFIDENCE_THRESHOLD", "0.6")
        )  # DEBUG: Lowered for testing
        self.staff_match_threshold = float(
            os.getenv("STAFF_MATCH_THRESHOLD", "0.75")
        )  # DEBUG: Lowered for testing
        self.mock_mode = (
            os.getenv("MOCK_MODE", "false").lower() == "true"
        )  # DEBUG: Disable mock mode
        ...
        self.max_camera_reconnect_attempts = int(
            os.getenv("MAX_CAMERA_RECONNECT_ATTEMPTS", "5")
        )
        self.failed_event_retry_interval = int(
            os.getenv("FAILED_EVENT_RETRY_INTERVAL", "30")
        )
    ...
    def validate(self):
        ...
        if not self.worker_api_key or self.worker_api_key == "dev-secret":
            logger.warning(
                "Using default API key - this should be changed in production"
            )

        if self.mock_mode:
            logger.info(
                "Running in MOCK MODE - no real cameras or ML models will be used"
            )
        ...
        if self.worker_fps > 10:
            logger.warning(
                f"High FPS setting ({self.worker_fps}) may impact performance"
            )

        logger.info(
            f"Worker configured: {self.detector_type} detector, {self.embedder_type} embedder, {self.worker_fps} FPS"
        )
...
class FaceRecognitionWorker:
    ...
    async def initialize(self):
        ...
        if self.access_token:
            await self.webrtc_streamer.initialize(
                self.access_token, self.config.api_url
            )
        ...
        self.queue_processor_task = asyncio.create_task(
            self._process_failed_events_queue()
        )
    ...
    async def shutdown(self):
        ...
        if (
            hasattr(self, "shutdown_monitor_task")
            and self.shutdown_monitor_task
            and not self.shutdown_monitor_task.done()
        ):
    ...
    async def _load_staff_embeddings(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Loaded {len(self.staff_embeddings)} staff embeddings for local matching"
                )
            else:
                logger.warning(
                    f"Failed to fetch staff embeddings: {response.status_code}"
                )
    ...
    def _is_staff_match(
        ...
        try:
            ...
            if matched_staff_id:
                logger.info(
                    f"Matched staff member {matched_staff_id} with similarity {best_similarity:.3f}"
                )
            ...
            try:
                ...
                while not self.failed_events_queue.empty():
                    try:
                        ...
                        result = await self._send_face_event(
                            event, face_image_bytes, max_retries=1
                        )

                        if "error" not in result:
                            processed += 1
                            logger.info(
                                f"Successfully processed queued event: {result}"
                            )
                        elif retry_count < self.config.max_queue_retries:
                            failed_again.append(
                                (event, face_image_bytes, retry_count + 1)
                            )
                        else:
                            logger.error(
                                f"Permanently dropping event after {self.config.max_queue_retries} retries: {event.camera_id}"
                            )
                ...
                if processed > 0:
                    logger.info(
                        f"Processed {processed} queued events, {len(failed_again)} still pending"
                    )
    ...
    async def _upload_face_image(
        self, face_image: np.ndarray, max_retries: int = 3
    ) -> Optional[bytes]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not success:
                    logger.warning(
                        f"Failed to encode face image (attempt {attempt + 1}/{max_retries})"
                    )
                    if attempt < max_retries - 1:
                ...
                if (
                    len(img_bytes) < 1000 or len(img_bytes) > 10 * 1024 * 1024
                ):  # 1KB to 10MB
                    logger.warning(f"Face image size unusual: {len(img_bytes)} bytes")

                logger.debug(
                    f"Successfully encoded face image ({len(img_bytes)} bytes) (attempt {attempt + 1})"
                )
                return img_bytes

            except Exception as e:
                logger.error(
                    f"Unexpected encode error (attempt {attempt + 1}/{max_retries}): {e}"
                )
                if attempt < max_retries - 1:
    ...
    async def _send_face_event(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if face_image_bytes:
                    ...
                    files = {"face_image": ("face.jpg", face_image_bytes, "image/jpeg")}
                ...
                logger.info(
                    f"Face event processed successfully: person_id={result.get('person_id')}, match={result.get('match')}"
                )
                return result
                ...
                if e.response.status_code == 401:
                    ...
                    # Server error, retry
                    logger.warning(
                        f"Server error (attempt {attempt + 1}/{max_retries + 1}): {e.response.status_code}"
                    )
                else:
        ...
        # All retries failed
        logger.error(
            f"Failed to send face event after {max_retries + 1} attempts, queuing for later retry"
        )

        # Add to failed events queue for later processing
        try:
            await self.failed_events_queue.put(
                (event, face_image_bytes, 1)
            )  # (event, face_image_bytes, retry_count)
    ...
    async def _monitor_shutdown(self):
        ...
        while True:
            try:
                if (
                    self.worker_client.should_shutdown()
                    and not self._shutdown_requested
                ):
                    logger.info(
                        "Shutdown monitor detected shutdown request - setting shutdown flag"
                    )
                    ...
                    if self._shutdown_requested:
                        logger.warning(
                            "Graceful shutdown period expired, completing shutdown..."
                        )
    ...
    async def _send_stop_signal_to_backend(self):
        ...
        for attempt in range(max_retries):
            try:
                logger.info(
                    f"🚪 Sending stop signal to backend (attempt {attempt + 1}/{max_retries})"
                )
                ...
                backend_cleanup_completed = response_data.get(
                    "backend_cleanup_completed", False
                )
                camera_released = response_data.get("camera_released", False)

                logger.info(
                    f"✅ Successfully sent stop signal to consolidated API (attempt {attempt + 1})"
                )
                logger.info(
                    f"📄 Backend response: {response_data.get('message', 'No message')}"
                )

                if backend_cleanup_completed:
                    logger.info(
                        "🎉 Backend confirmed cleanup completed - safe to shutdown"
                    )
                    if camera_released:
                    ...
                    logger.info(
                        "⚠️  Backend cleanup status unknown - proceeding with shutdown"
                    )
                ...
                logger.warning(
                    f"⏱️  Timeout sending stop signal (attempt {attempt + 1}/{max_retries})"
                )
            except httpx.TimeoutException:
                logger.warning(
                    f"⏱️  Request timeout sending stop signal (attempt {attempt + 1}/{max_retries})"
                )
            except Exception as e:
                logger.warning(
                    f"❌ Error sending stop signal (attempt {attempt + 1}/{max_retries}): {e}"
                )
        ...
        logger.error(
            f"💥 Failed to send stop signal to backend after {max_retries} attempts - proceeding with local cleanup"
        )
        return False
        ...
        try:
            ...
            for detection in detections:
                ...
                if not assigned_camera_id:
                    logger.warning(
                        "No camera assigned to worker, skipping face processing"
                    )
                    continue
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.info(
                        f"Camera connected successfully, processing at {self.config.worker_fps} FPS"
                    )
                    ...
                    while True:
                        ...
                        if frame_count % 10 == 0 or self.should_shutdown():
                            if self.should_shutdown():
                                logger.info(
                                    "Shutdown signal received, stopping camera capture"
                                )
                                break
                        ...
                        if current_time - last_process_time >= frame_delay:
                            try:
                                ...
                                if faces_count > 0:
                                    logger.info(
                                        f"Processed {faces_count} faces at {datetime.now().strftime('%H:%M:%S')}"
                                    )
                    ...
                    await self.worker_client.report_error(
                        f"Camera error: {str(camera_error)}"
                    )
                    ...
                    logger.info(
                        f"Waiting {wait_time}s before reconnection attempt {reconnect_attempts}"
                    )
                    await asyncio.sleep(wait_time)
...
async def main():
    ...
    def signal_handler(signum, frame):
        ...
        if shutdown_start_time is None:
            ...
            logger.warning(
                f"Second signal received after {elapsed:.1f}s - forcing immediate exit"
            )
            os._exit(1)
