...
class WorkerIDManager:
        ...
        try:
            ...
            if dotenv_worker_id and dotenv_worker_id.strip():
                ...
                self._save_worker_id(
                    worker_id, tenant_id, site_id, worker_id_suffix=worker_id
                )
                return worker_id
            ...
            if existing_id:
                logger.info(
                    f"Loaded existing worker ID from persistent storage: {existing_id}"
                )
                return existing_id
    ...
    def _get_worker_id_file(
        self, tenant_id: str, worker_id_suffix: Optional[str] = None
    ) -> Path:
        ...
        if worker_id_suffix:
            ...
            safe_worker_suffix = (
                worker_id_suffix.replace("/", "_").replace("\\", "_").replace(":", "_")
            )
            return (
                self.data_dir / f"worker_id_{safe_tenant_id}_{safe_worker_suffix}.json"
            )
        else:
                        ...
                        with open(env_file, "r") as f:
                            for line in f:
                                ...
                                if line.startswith(
                                    ...
                                    if (
                                        worker_id.startswith('"')
                                        and worker_id.endswith('"')
                                    ) or (
                                        worker_id.startswith("'")
                                        and worker_id.endswith("'")
                                    ):
                                    ...
                                    if worker_id:
                                        logger.debug(
                                            f"Found WORKER_ID in {env_file}: {worker_id}"
                                        )
                                        return worker_id
    ...
    def _load_worker_id(
        ...
        try:
            ...
            if (
                data.get("hostname") == self.hostname
                ...
                if (
                    site_id is None
                    or stored_site_id is None
                    or stored_site_id == site_id
                ):
                    worker_id = data.get("worker_id")
                    if worker_id:
                        logger.debug(
                            f"Found valid worker ID for hostname={self.hostname}, tenant={tenant_id}"
                        )
                        return worker_id

            logger.info(
                f"Stored worker ID doesn't match current context (hostname={self.hostname}, tenant={tenant_id})"
            )
            return None
    ...
    def update_last_used(
        self, worker_id: str, tenant_id: str, site_id: Optional[str] = None
    ):
        ...
        for worker_id_file in worker_id_files:
            try:
                ...
                if data.get("worker_id") == worker_id:
                    ...
                    logger.debug(
                        f"Updated last used timestamp for worker ID {worker_id}"
                    )
                    return  # Successfully updated, exit

            except (json.JSONDecodeError, OSError) as e:
                logger.debug(
                    f"Failed to update last used timestamp in {worker_id_file}: {e}"
                )
                continue
