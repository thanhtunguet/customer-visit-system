...
from apps.worker.app.main import WorkerConfig, main
    ...
    with patch.dict(os.environ, test_env):
        ...
        with patch("apps.worker.app.main.config") as mock_config:
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                assert (
                    mock_client_instance.post.call_count >= 2
                )  # At least auth + face event
...
async def test_worker_graceful_shutdown():
    ...
    with patch("httpx.AsyncClient") as mock_client:
        ...
        # Verify cleanup
        assert (
            worker.queue_processor_task.cancelled()
            or worker.queue_processor_task.done()
        )
