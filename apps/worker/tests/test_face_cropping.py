...
class TestFaceCropper:
    ...
    def test_initialization_default_values(self):
        ...
        assert cropper.preserve_aspect
    ...
    def test_initialization_custom_values(self):
        ...
        assert not cropper.preserve_aspect
    ...
    def test_crop_large_face(self, sample_image, large_face):
        ...
        assert result["cropped_face"].shape == (
            cropper.target_size,
            cropper.target_size,
            3,
        )
    ...
    def test_crop_edge_face(self, sample_image, edge_face):
        ...
        assert (
            crop_bbox[0] + crop_bbox[2] <= sample_image.shape[1]
        )  # x + w <= img_width
        assert (
            crop_bbox[1] + crop_bbox[3] <= sample_image.shape[0]
        )  # y + h <= img_height
    ...
    def test_multiple_faces_most_centered_strategy(self, sample_image):
        ...
        assert (
            result["selected_face_index"] == 1
        )  # Should select the most centered face
    ...
    def test_validate_crop_result(self):
        ...
        # Valid crop result
        valid_result = {"cropped_face": np.ones((100, 100, 3), dtype=np.uint8)}
        assert cropper.validate_crop_result(valid_result)

        # Invalid crop results
        assert not cropper.validate_crop_result({})
        assert not cropper.validate_crop_result({"cropped_face": None})
        assert not cropper.validate_crop_result({"cropped_face": np.array([])})

        # Too small crop
        tiny_crop = {"cropped_face": np.ones((10, 10, 3), dtype=np.uint8)}
        assert not cropper.validate_crop_result(tiny_crop)
...
class TestFaceCropperIntegration:
    ...
    def create_synthetic_face_image(
        self, face_size_ratio: float, position: str = "center"
        ...
        cv2.circle(
            image, (x + face_w // 3, y + face_h // 3), face_w // 10, (50, 50, 50), -1
        )  # Left eye
        cv2.circle(
            image,
            (x + 2 * face_w // 3, y + face_h // 3),
            face_w // 10,
            (50, 50, 50),
            -1,
        )  # Right eye
    ...
    def test_huge_face_scenario(self):
        ...
        image, face_info = self.create_synthetic_face_image(
            0.8, "center"
        )  # 80% of image
    ...
    def test_edge_case_scenarios(self):
        ...
        scenarios = [("edge", 0.15), ("corner", 0.12)]
    ...
    def test_multiple_face_scenarios(self):
        ...
        small_face = {"bbox": [500, 300, 60, 70], "confidence": 0.7, "landmarks": None}
