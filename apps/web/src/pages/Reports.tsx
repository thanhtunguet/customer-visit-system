...
export const Reports: React.FC = () => {
  ...
  const [realDemographics, setRealDemographics] = useState<{
    visitorType?: Array<{ name: string; value: number; color: string }>;
    ...
    try {
      ...
      if (sites.length > 0) {
        try {
          ...
          const filteredSiteData = siteResults.filter(
  ...
  const handleExport = (
    type: 'visitor-trends' | 'demographics' | 'site-comparison'
  ) => {
                    ...
                    <PieChart>
                      ...
                      >
                        {demographics.visitorType?.map((entry, index) => (
                          <Cell
                            key={`visitor-type-${index}-${entry.name}`}
                            fill={entry.color}
                          />
                        ))}
                      </Pie>
