...
import { Table, Button, Modal, Form, Typography, Space, Alert, Tag, Select, Popconfirm, Tooltip, App, } from 'antd';
...
export const Cameras = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'camera_type',
            render: (type) => (_jsx(Tag, { color: type === 'rtsp' ? 'blue' : 'green', children: type === 'rtsp' ? 'RTSP' : 'Webcam' })),
        },
        {
            title: 'RTSP URL',
            dataIndex: 'rtsp_url',
        ...
        },
        {
            ...
            key: 'device_index',
            render: (index) => index !== null && index !== undefined ? (_jsx(Tooltip, { title: `OpenCV device index ${index} - matches system webcam enumeration`, children: _jsxs("span", { className: "font-mono bg-gray-100 px-2 py-1 rounded", children: ["#", index] }) })) : (_jsx("span", { className: "text-gray-400", children: "-" })),
        },
        {
            title: 'Status',
            dataIndex: 'is_active',
        ...
        },
        {
            ...
            render: (_, camera) => {
                ...
                return (_jsx(Tag, { color: isStreaming ? 'blue' : 'default', children: isStreaming ? 'Streaming' : 'Not Streaming' }));
            },
        },
        {
            ...
            render: (_, camera) => {
                ...
                return (_jsx(Tag, { color: isProcessing ? 'orange' : 'default', children: isProcessing ? 'Processing Faces' : 'Not Processing' }));
            },
        },
        {
            title: 'Created',
        ...
        },
        {
            ...
            render: (_, camera) => {
                const isStreaming = streamStatuses[camera.camera_id];
                const isProcessing = processingStatuses[camera.camera_id];
                return (_jsxs(Space, { size: "small", children: [_jsx(Tooltip, { title: "View WebRTC P2P stream", children: _jsx(Button, { type: "text", icon: _jsx(WifiOutlined, {}), onClick: () => handleViewStream(camera), size: "small", disabled: !camera.is_active, style: { color: '#1890ff' } }) }), !isStreaming ? (_jsx(Tooltip, { title: "Start WebRTC P2P stream", children: _jsx(Button, { type: "text", icon: _jsx(PlayCircleOutlined, {}), onClick: () => handleStartStreaming(camera), size: "small", disabled: !camera.is_active, style: { color: '#1890ff' } }) })) : (_jsx(Tooltip, { title: "Stop WebRTC stream", children: _jsx(Popconfirm, { title: "Stop Stream", description: "Stop WebRTC P2P stream?", onConfirm: () => handleStopStream(camera), okText: "Yes", cancelText: "No", children: _jsx(Button, { type: "text", icon: _jsx(StopOutlined, {}), size: "small", danger: true }) }) })), !isProcessing ? (_jsx(Tooltip, { title: "Start face recognition processing", children: _jsx(Button, { type: "text", icon: _jsx(RobotOutlined, {}), onClick: () => handleStartProcessing(camera), size: "small", disabled: !camera.is_active, style: { color: '#52c41a' } }) })) : (_jsx(Tooltip, { title: "Stop face recognition processing", children: _jsx(Popconfirm, { title: "Stop Processing", description: "Are you sure you want to stop face recognition processing?", onConfirm: () => handleStopProcessing(camera), okText: "Yes", cancelText: "No", children: _jsx(Button, { type: "text", icon: _jsx(PauseOutlined, {}), size: "small", style: { color: '#fa8c16' } }) }) })), _jsx(EditAction, { onClick: () => handleEditCamera(camera), tooltip: "Edit camera" }), _jsx(DeleteAction, { onConfirm: () => handleDeleteCamera(camera), title: "Delete Camera", description: "Are you sure you want to delete this camera?", tooltip: "Delete camera" })] }));
            },
        },
    ];
    if (error && cameras.length === 0) {
        return (_jsx(Alert, { message: "Error Loading Cameras", description: error, type: "error", showIcon: true, action: _jsx(Button, { onClick: () => selectedSite && loadCameras(selectedSite), children: "Retry" }) }));
    }
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx(Title, { level: 2, className: "mb-0", children: "Cameras" }), _jsxs(Space, { children: [_jsx(Select, { value: selectedSite, onChange: setSelectedSite, placeholder: "Select Site", style: { width: 200 }, options: sites.map((site) => ({
                                    value: site.site_id,
                                    label: site.name,
                                })) }), _jsx(Button, { type: "primary", icon: _jsx(PlusOutlined, {}), onClick: () => {
                                    setEditingCamera(null);
                                    form.resetFields();
                                    ...
                                    setModalVisible(true);
                                }, className: "bg-blue-600", disabled: !selectedSite, children: "Add Camera" }), _jsxs(Button, { icon: _jsx(EyeOutlined, {}), onClick: handleViewAllStreams, disabled: !selectedSite ||
                                    cameras.filter((cam) => streamStatuses[cam.camera_id]).length ===
                                        0, children: ["View All Streams (", cameras.filter((cam) => streamStatuses[cam.camera_id]).length, ")"] })] })] }), error && (_jsx(Alert, { message: error, type: "error", closable: true, onClose: () => setError(null) })), _jsx("div", { className: "bg-white rounded-lg shadow", children: _jsx(Table, { columns: columns, dataSource: cameras, rowKey: "camera_id", loading: loading, pagination: {
                        total: cameras.length,
                        pageSize: 10,
                        showSizeChanger: true,
                        showQuickJumper: true,
                        showTotal: (total) => `Total ${total} cameras`,
                    } }) }), _jsx(Modal, { title: editingCamera ? 'Edit Camera' : 'Add New Camera', open: modalVisible, onCancel: () => {
                    setModalVisible(false);
                    setEditingCamera(null);
                    form.resetFields();
                }, onOk: () => form.submit(), confirmLoading: loading, children: _jsx(Form, { form: form, layout: "vertical", onFinish: handleCreateCamera, children: _jsx(CameraForm, { form: form, selectedSite: selectedSite }) }) }), _jsx(Modal, { title: _jsxs(Space, { children: [_jsx(WifiOutlined, { style: { color: '#1890ff' } }), _jsxs("span", { children: [streamingCamera?.name, " - WebRTC P2P Stream"] }), _jsxs("span", { style: { color: '#8c8c8c', fontSize: '14px' }, children: ["#", streamingCamera?.camera_id] }), _jsx(Tag, { color: "blue", style: { marginLeft: '8px' }, children: "Peer-to-Peer" })] }), open: streamModalVisible, onCancel: () => {
                    setStreamModalVisible(false);
                    setStreamingCamera(null);
                    setStreamConnectionState('disconnected');
                    setStreamIntent('view');
                }, footer: null, width: "95%", style: { maxWidth: '1200px' }, styles: { body: { padding: '8px' } }, centered: true, children: streamingCamera && (_jsx(WebRTCCameraStream, { siteId: selectedSite, cameraId: streamingCamera.camera_id, cameraName: streamingCamera.name, onClose: () => {
                        setStreamModalVisible(false);
                        setStreamingCamera(null);
                    }, onStreamStateChange: handleStreamStateChange, onConnectionStateChange: () => { }, autoStart: streamIntent === 'start', autoReconnect: streamIntent === 'view', currentStreamStatus: streamStatuses[streamingCamera.camera_id] || false }, `webrtc-${selectedSite}-${streamingCamera.camera_id}`)) }), _jsx(Modal, { title: null, open: multiStreamModalVisible, onCancel: handleCloseMultiStreamModal, footer: null, width: "98%", style: { maxWidth: '1920px' }, styles: { body: { padding: '0' } }, centered: true, destroyOnHidden: true, children: _jsx(MultiCameraStreamView, { siteId: selectedSite, cameras: cameras, streamStatuses: streamStatuses, onStreamStateChange: handleStreamStateChange, onStopStream: handleStopStream }, `multi-stream-${selectedSite}-${multiStreamModalVisible}`) })] }));
};
