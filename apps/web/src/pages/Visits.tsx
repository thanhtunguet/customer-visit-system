...
const seedTime = (minutesAgo: number) =>
...
export const VisitsPage: React.FC = () => {
  ...
  const [primaryVisitId, setPrimaryVisitId] = useState<string | undefined>(
    undefined
  );
    ...
    <div className="p-6">
          ...
          <Row gutter={16} align="middle">
            <Col>
              <Checkbox
                ...
                checked={
                  visits.length > 0 && selectedVisitIds.size === visits.length
                }
                  ...
                  >
                    ...
                    >
                      <Checkbox
                        ...
                        onChange={(e) =>
                          handleSelectVisit(visit.visit_id, e.target.checked)
                        }
                      />
                    </div>
                    <div className="flex items-center justify-between mb-2 pr-8">
                      <Tag color={getPersonTypeColor(visit.person_type)}>
                        {visit.person_type.charAt(0).toUpperCase() +
                    ...
                    <div className="flex flex-col items-center mb-3">
                      ...
                      <div className="text-center">
                        <Text strong>
                          {visit.person_type === 'staff' ? 'Staff' : 'Customer'}{' '}
                          #{visit.person_id}
                        </Text>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto">
                      ...
                      <div className="mt-1">
                        <Text type="secondary" className="text-xs">
                          Confidence:{' '}
                          {(visit.confidence_score * 100).toFixed(1)}%
                        </Text>
                      </div>
                      ...
                      {visit.visit_duration_seconds &&
                        visit.visit_duration_seconds > 0 && (
                          <div className="mt-1">
                            <Text type="secondary" className="text-xs">
                              Duration:{' '}
                              {Math.floor(visit.visit_duration_seconds / 60)}m{' '}
                              {visit.visit_duration_seconds % 60}s
                            </Text>
                          </div>
                        )}
                    </div>
            ...
            <div className="w-full">
              ...
              <div className="mb-3 text-center">
                <Text strong className="text-lg">
                  {selectedVisit.person_type === 'staff' ? 'Staff' : 'Customer'}{' '}
                  #{selectedVisit.person_id}
                </Text>
                <br />
                <Text type="secondary">Visit ID: {selectedVisit.visit_id}</Text>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center mb-2">
                  ...
                  <Text>
                    <strong>Date:</strong>{' '}
                    {new Date(selectedVisit.timestamp).toLocaleDateString()}
                  </Text>
                </div>
                ...
                <div className="flex items-center mb-2">
                  ...
                  <Text>
                    <strong>Time:</strong>{' '}
                    {new Date(selectedVisit.timestamp).toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}
                  </Text>
                  ...
                  selectedVisit.visit_duration_seconds > 0 && (
                    <div className="flex items-center mb-2">
                      <Text>
                        ...
                        {Math.floor(selectedVisit.visit_duration_seconds / 60)}m{' '}
                        {selectedVisit.visit_duration_seconds % 60}s
                      </Text>
                    </div>
                  )}
                ...
                {selectedVisit.highest_confidence &&
                  selectedVisit.highest_confidence !==
                    ...
                    <div className="flex items-center">
                      <Text>
                        <strong>Peak Confidence:</strong>{' '}
                        {(selectedVisit.highest_confidence * 100).toFixed(1)}%
                      </Text>
                    </div>
                  )}
              </div>
                      ...
                      <Popconfirm
                        ...
                        onConfirm={async () => {
                          try {
                            ...
                            const error = e as ApiError;
                            message.error(
                              error.response?.data?.detail || 'Failed to remove'
                            );
                          }
                        }}
                      >
      ...
      >
        <div className="space-y-3">
          <div>Choose primary visit (keeps best image/fields):</div>
          <Select
            className="w-full"
          ...
          >
            {selectedVisits.map((v) => (
              <Select.Option key={v.visit_id} value={v.visit_id}>
                {v.visit_id} • {new Date(v.first_seen).toLocaleString()} • conf{' '}
                {(100 * (v.highest_confidence ?? v.confidence_score)).toFixed(
                  1
                )}
                %
              </Select.Option>
          ...
          <div className="text-xs text-gray-500">
            All visits must belong to the same person and site. The primary
            visit will absorb the others.
          </div>
        </div>
      </Modal>
    </div>
