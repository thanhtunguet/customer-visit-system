...
import { PlayCircleOutlined, StopOutlined, ReloadOutlined, WifiOutlined, } from '@ant-design/icons';
import { apiClient } from '../services/api';
export const WebRTCCameraStream = ({ siteId, cameraId, cameraName: _cameraName, onClose: _onClose, autoStart = false, autoReconnect: _autoReconnect = false, currentStreamStatus = false, onStreamStateChange, onConnectionStateChange, }) => {
    ...
    const notifyStreamStateChange = useCallback((isActive) => {
    ...
    const notifyConnectionStateChange = useCallback((state) => {
    ...
    const getConnectionStatusColor = () => {
        ...
        if (connectionState === 'connecting' || signalingState === 'connecting')
            return 'blue';
        if (connectionState === 'error' || signalingState === 'error')
            return 'red';
        return 'default';
    };
    return (_jsxs("div", { className: "space-y-3", children: [error && (_jsx(Alert, { message: "WebRTC Stream Error", description: error, type: "error", closable: true, onClose: () => setError(null) })), _jsx("div", { className: "relative bg-gray-900 rounded-lg overflow-hidden w-full", style: { aspectRatio: '16/9' }, children: isStreaming ? (_jsxs(_Fragment, { children: [_jsx("video", { ref: videoRef, autoPlay: true, playsInline: true, muted: true, className: "w-full h-full object-contain", 
                            ...
                            } }), connectionState !== 'connected' && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center", children: _jsxs("div", { className: "text-center text-white", children: [connectionState === 'connecting' && (_jsxs(_Fragment, { children: [_jsx(Spin, { size: "large" }), _jsx("div", { className: "mt-4", children: "Establishing P2P connection..." }), signalingState !== 'open' && (_jsx("div", { className: "text-sm text-gray-400 mt-2", children: "Connecting to signaling server..." }))] })), connectionState === 'error' && (_jsxs(_Fragment, { children: [_jsx("div", { className: "text-red-400 text-lg mb-2", children: "P2P Connection Failed" }), _jsxs(Button, { onClick: handleManualReconnect, loading: loading, children: [_jsx(ReloadOutlined, {}), " Reconnect"] }), _jsxs("div", { className: "text-sm text-gray-400 mt-2", children: ["Retry ", retryCountRef.current, "/", maxRetries] })] }))] }) }))] })) : (_jsx("div", { className: "absolute inset-0 flex items-center justify-center bg-gray-800", children: _jsxs("div", { className: "text-center text-white", children: [_jsx(WifiOutlined, { className: "text-4xl mb-4 text-gray-400" }), _jsx("div", { className: "text-lg", children: "WebRTC P2P Stream" }), _jsx("div", { className: "text-sm text-gray-400 mt-2", children: "Direct connection to worker camera" })] }) })) }), _jsxs("div", { className: "flex justify-between items-center", children: [_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(Tag, { color: getConnectionStatusColor(), children: getConnectionStatus() }), session && (_jsx(Tooltip, { title: `Session: ${session.session_id.substring(0, 8)}...`, children: _jsx(Tag, { color: "purple", children: "WebRTC" }) }))] }), _jsx(Space, { size: "small", children: !isStreaming ? (_jsx(Button, { type: "primary", icon: _jsx(PlayCircleOutlined, {}), onClick: handleStartStream, loading: loading, size: "middle", children: "Start WebRTC Stream" })) : (_jsxs(_Fragment, { children: [_jsx(Popconfirm, { title: "Stop WebRTC Stream", description: "Are you sure you want to stop the peer-to-peer stream?", onConfirm: handleStopStream, okText: "Yes, Stop", cancelText: "Cancel", okButtonProps: { danger: true }, children: _jsx(Button, { icon: _jsx(StopOutlined, {}), loading: loading, size: "middle", danger: true, children: "Stop Stream" }) }), connectionState !== 'connected' && (_jsx(Button, { icon: _jsx(ReloadOutlined, {}), onClick: handleManualReconnect, loading: loading, size: "middle", children: "Reconnect" }))] })) })] }), isStreaming && (_jsxs("div", { className: "text-xs text-gray-500 text-center space-x-3", children: [_jsxs("span", { children: ["Client: ", currentClientId] }), _jsxs("span", { children: ["Signaling: ", signalingState] }), _jsxs("span", { children: ["P2P: ", connectionState] }), retryCountRef.current > 0 && (_jsxs("span", { children: ["Retries: ", retryCountRef.current, "/", maxRetries] }))] }))] }));
};
