...
export const CustomerFaceGallery = ({
  customerId,
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      const response = await apiClient.deleteCustomerFaceImagesBatch(
        customerId,
        imageIds
      );
      ...
      message.success(
        `Successfully deleted ${response.deleted_count} face image${response.deleted_count > 1 ? 's' : ''}`
      );
  ...
  const getImageTooltip = (image) =>
    _jsxs('div', {
      className: 'space-y-2 max-w-xs text-white',
      children: [
        _jsxs('div', {
        ...
        _jsxs('div', {
          ...
          children: [
            ...
            _jsxs('span', {
              className: `px-2 py-0.5 rounded text-xs font-medium ${image.confidence_score > 0.8 ? 'bg-green-600 text-white' : 'bg-orange-600 text-white'}`,
              children: [(image.confidence_score * 100).toFixed(1), '%'],
            }),
          ],
        }),
        image.quality_score &&
          _jsxs('div', {
            ...
            children: [
              ...
              _jsxs('span', {
                className: `px-2 py-0.5 rounded text-xs font-medium ${image.quality_score > 0.8 ? 'bg-green-600 text-white' : 'bg-orange-600 text-white'}`,
                children: [(image.quality_score * 100).toFixed(1), '%'],
              }),
            ],
          }),
        _jsxs('div', {
          ...
          children: [
            ...
            _jsx('span', {
              ...
              children: dayjs(image.created_at).format('MMM D, YYYY HH:mm'),
            }),
          ],
        }),
        image.visit_id &&
          _jsxs('div', {
            ...
            children: [
              ...
              _jsx('span', {
                ...
                children: image.visit_id.slice(-8),
              }),
            ],
          }),
        image.face_bbox &&
          image.face_bbox.length >= 4 &&
          _jsxs('div', {
            className: 'flex justify-between items-center',
            children: [
              _jsx('span', {
              ...
              _jsxs('span', {
                className: 'text-xs text-gray-300',
                children: [
                  Math.round(image.face_bbox[2]),
                  '\u00D7',
                  Math.round(image.face_bbox[3]),
                  'px',
                ],
              }),
            ],
        ...
        children: _jsx(Card, {
          ...
          actions: [
            _jsx(
              Tooltip,
              {
                title: getImageTooltip(image),
                children: _jsx(InfoCircleOutlined, {
                  onClick: (e) => {
                    e.stopPropagation();
                  },
      ...
      children: [
                ...
                _jsxs(Space, {
                  children: [
                    ...
                    _jsxs('span', {
                      ...
                      children: ['Face Gallery (', images.length, ' images)'],
                    }),
                  ],
                }),
                images.length > 0 &&
        ...
        images.length > 1 &&
          _jsx(Alert, {
            ...
            description:
              'Click to select \u2022 Ctrl/\u2318+Click to add to selection \u2022 Shift+Click to select range',
            type: 'info',
            showIcon: true,
            className: 'text-sm',
          }),
        images.length === 0
          ...
          : _jsx(Row, { gutter: [16, 16], children: images.map(renderImage) }),
      ],
    }),
  });
};
