...
interface WebRTCCameraStreamProps {
  ...
  onStreamStateChange?: (cameraId: number | string, isActive: boolean) => void;
...
export const WebRTCCameraStream: React.FC<WebRTCCameraStreamProps> = ({
  ...
  const [signalingState, setSignalingState] =
    useState<SignalingState>('closed');
  ...
  const handleSignalingMessageRef =
    useRef<
      (message: {
        type: string;
        data?: unknown;
  ...
  const setupSignalingConnection =
    useCallback(async (): Promise<WebSocket | null> => {
      try {
  ...
  const handleSignalingMessage = useCallback(
    async (message: {
      ...
      switch (message.type) {
        ...
        case 'signaling': {
          const signalingData = message.data as SignalingData;
    ...
    <div className="space-y-3">
        ...
        {isStreaming ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gray-800">
            <div className="text-center text-white">
              ...
              <div className="text-sm text-gray-400 mt-2">
                Direct connection to worker camera
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          <Tag color={getConnectionStatusColor()}>{getConnectionStatus()}</Tag>

          {session && (
            <Tooltip
              title={`Session: ${session.session_id.substring(0, 8)}...`}
            >
              <Tag color="purple">WebRTC</Tag>
            </Tooltip>
          )}
        </div>
