...
export const BulkCustomerMergeModal: React.FC<BulkCustomerMergeModalProps> = ({
  ...
  const handleCreateNewGroup = () => {
    ...
    const usedCustomerIds = new Set(
      mergeGroups.flatMap(g => [g.primary.customer_id, ...g.secondaries.map(s => s.customer_id)])
    );
  ...
  const renderCustomerItem = (customer: Customer, isPrimary: boolean = false) => (
    <div className="flex items-center space-x-3 p-2 bg-gray-50 rounded border">
      ...
      <div className="flex-1">
        <div className="font-medium">
          ...
          {customer.name && <span className="ml-2 text-gray-600">{customer.name}</span>}
          {isPrimary && <Tag color="blue" className="ml-2">PRIMARY</Tag>}
        </div>
        <div className="text-sm text-gray-500">
          {customer.visit_count} visits • Last seen: {dayjs(customer.last_seen).format('MMM D, YYYY')}
        </div>
      </div>
    </div>
  );
    ...
    >
      <div className="space-y-4">
        <Alert
          ...
          description={
            <div>
              <p>You have selected <strong>{getTotalCustomers()}</strong> customers for bulk merging.</p>
              <p>Configure merge groups below. Each group will merge secondary customers into a primary customer.</p>
        ...
        <div className="bg-blue-50 p-3 rounded border">
          <div className="text-sm font-medium text-blue-800 mb-1">Merge Summary</div>
          <div className="text-sm text-blue-700">
            <span className="mr-4">Total Customers: {getTotalCustomers()}</span>
            <span className="mr-4">Will be merged: {getTotalMergeTargets()}</span>
            ...
            <div key={groupIndex} className="border rounded p-4 bg-white">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button
                  size="small"
                  danger
                  onClick={() => setMergeGroups(groups => groups.filter((_, i) => i !== groupIndex))}
                >
                  Remove Group
                </Button>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <div className="text-sm font-medium text-gray-700 mb-2">
                    Secondary Customers ({group.secondaries.length}) - will be merged into primary
                  </div>
                  <div className="space-y-2">
                    {group.secondaries.map(customer => (
                      <div key={customer.customer_id} className="flex items-center space-x-2">
                        <div className="flex-1">
                        ...
                        <Button
                          size="small"
                          danger
                          onClick={() => handleRemoveFromGroup(groupIndex, customer.customer_id)}
                        >
                          Remove
                        </Button>
                      </div>
        ...
        <div className="flex justify-center">
          <Button
            ...
            disabled={
              mergeGroups.reduce((used, g) => used + 1 + g.secondaries.length, 0) >= selectedCustomers.length
            }
          >
            Create New Group
          </Button>
