...
export const FaceRecognitionTest: React.FC<FaceRecognitionTestProps> = ({
  ...
  const [testResult, setTestResult] =
    useState<FaceRecognitionTestResult | null>(null);
  ...
  const runTest = async () => {
    ...
    try {
      ...
      setError(
        (error as ApiError)?.response?.data?.detail || 'Recognition test failed'
      );
      setTestResult(null);
    } finally {
      setTesting(false);
  ...
  const columns: ColumnsType<FaceRecognitionMatch> = [
    {
      title: 'Rank',
      dataIndex: 'rank',
      key: 'rank',
      ...
      render: (similarity: number) => (
        <Space direction="vertical" size="small" className="w-full">
          ...
          <div className="flex items-center justify-between">
            ...
            <Tag color={getSimilarityTagColor(similarity)} className="text-xs">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {testImage && <Button onClick={clearTest}>Clear Test</Button>}
      </div>
        ...
        <div className="text-center space-y-2">
          {!testImageUrl ? (
            ...
            >
              ...
              <p className="ant-upload-text">Click or drag a test image here</p>
              <p className="ant-upload-hint">
                Supports JPG, PNG, GIF formats. Best results with clear face
                photos.
              </p>
            </Upload.Dragger>
          ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
              ...
              <div>
                <div className="text-2xl font-bold text-green-600">
                  {(testResult.processing_info.test_confidence * 100).toFixed(
                    1
                  )}
                  %
                </div>
                  ...
                  <div className="mt-1">
                    <Space>
                      <span className="font-medium">
                        {testResult.best_match.staff_name}
                      </span>
                      <Tag color="blue">
                        ID: {testResult.best_match.staff_id}
                      </Tag>
              ...
              <div className="space-y-2 text-sm">
                {testResult.best_match?.staff_id === staffId ? (
                  ...
                  <div className="flex items-center space-x-2 text-red-600">
                    <CloseCircleOutlined />
                    <span>
                      Recognition failed. The system identified a different
                      person or no one above the confidence threshold.
                    </span>
                  </div>
                )}
                ...
                <div className="text-gray-600">
                  ...
                  {testResult.processing_info.test_confidence < 0.8 && (
                    <span>
                      {' '}
                      Try using a clearer image with better lighting and a
                      direct face view.
                    </span>
                  )}
                  {testResult.best_match &&
                    testResult.best_match.similarity < 0.7 && (
                      <span>
                        {' '}
                        Consider adding more face images to improve recognition
                        accuracy.
                      </span>
                    )}
                </div>
