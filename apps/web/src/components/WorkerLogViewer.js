...
const WorkerLogViewer = ({ visible, onClose, workerId, workerName }) => {
  ...
  const loadRecentLogs = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        'API call URL:',
        `/worker-management/workers/${workerId}/logs/recent?limit=100`
      );
      ...
      const errorMsg =
        axiosError.response?.data?.detail || 'Failed to load logs';
      setError(errorMsg);
      notification.error({
  ...
  const generateTestLogs = async () => {
    try {
      ...
      notification.error({
        message: 'Test Failed',
        description:
          axiosError.response?.data?.detail || 'Failed to generate test logs',
      });
    }
  };
  // Clear logs
  ...
  return _jsx(Modal, {
    title: _jsxs(Space, {
      children: [
        _jsx(BugOutlined, {}),
        _jsxs('span', { children: ['Worker Logs - ', workerName] }),
        _jsx(Tag, {
          color: streaming ? 'green' : 'default',
          children: streaming ? 'Streaming' : 'Offline',
        }),
      ],
    }),
    open: visible,
          ...
          children: [
                ...
                _jsxs(Space, {
                  children: [
                    _jsx(Button, {
                      ...
                      icon: streaming
                        ? _jsx(PauseCircleOutlined, {})
                        : _jsx(PlayCircleOutlined, {}),
                      onClick: streaming ? stopStreaming : startStreaming,
                      loading: loading,
                      children: streaming ? 'Stop Stream' : 'Start Stream',
                    }),
                    _jsx(Button, {
            ...
            _jsxs('div', {
              ...
              children: [
                _jsxs(Space, {
                  children: [
                    ...
                    _jsxs(Select, {
                      ...
                      size: 'small',
                      children: [
                        _jsx(Select.Option, { value: 'all', children: 'All' }),
                        _jsx(Select.Option, {
                          value: 'debug',
                          children: 'Debug',
                        }),
                ...
                _jsxs(Space, {
                  children: [
                    _jsx('span', { children: 'Auto-scroll:' }),
                    _jsx(Switch, {
                      checked: autoScroll,
        ...
        _jsx('div', {
          ...
          children: loading
            ...
            : _jsx('div', {
                ...
                className:
                  'h-full overflow-y-auto bg-black text-white font-mono text-sm',
                style: { padding: '12px' },
                children:
                  filteredLogs.length === 0
                    ? _jsxs('div', {
                        ...
                        children: [
                          'No log entries',
                          ' ',
                          logs.length > 0 ? 'match your filters' : 'available',
                        ],
                      })
                    : filteredLogs.map((log, index) =>
                        _jsxs(
                          ...
                          {
                            className:
                              'mb-1 leading-relaxed hover:bg-gray-800 px-2 py-1 rounded',
                            children: [
                              _jsx('span', {
                              ...
                              _jsx('span', { children: log.message }),
                              log.exception &&
                                _jsx('pre', {
                                  className:
                                    'text-red-400 mt-1 ml-4 text-xs whitespace-pre-wrap',
                                  children: log.exception,
                                }),
                            ],
                          },
                          `${log.timestamp}-${index}-${log.message.slice(0, 50)}`
                        )
                      ),
              }),
        }),
